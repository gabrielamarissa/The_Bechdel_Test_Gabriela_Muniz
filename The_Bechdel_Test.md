...
> The %&gt;% inside the code is called a pipe that feeds the code
> through line after line. I googled the answer.
...
> ‘summarise()’ creates a new dataframe. I googled the answer.
...
> ‘select()’ allows you to select specific variables from the data set.
> I googled it, and I figured from ‘select’ with the some not all the
...
Zooming in on the movies with `roi < 15` provides a better view of how
...
ggplot(data = bechdel90_13, mapping = aes(x = clean_test, y = roi, color = binary)) +
  geom_boxplot() +
  labs(title = "Return on investment vs. Bechdel test result",
       subtitle = "Focusing on Movies with roi, 15", # Something about zooming in to a certain level
       x = "Detailed Bechdel result",
       y = "Return on investment",
       color = "Binary Bechdel result") +
...
I learned that most movies from 1990-2013, failed the Bechdel test.
